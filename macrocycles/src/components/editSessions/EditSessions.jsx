...
export default function EditSessions ({
  ...
  useEffect(() => {
    setFunctionToExecute(() => async () => {
      try {
        setLoading(true)
        if (!checkTest && (newCategory.trim() === '' || newPlace.trim() === '' || newTrainner.trim() === '' || newObjectiveTec.trim() === '' || newObjectivePhysical.trim() === '' || newObjectiveEducational.trim() === '' || newMaterial.length === 0 || newStages.length === 0)) {
